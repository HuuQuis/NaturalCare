...
public class ExpertDAO extends DBContext {
    ...
    public boolean isUsernameTakenForRole(String username, int roleId) {
    ...
    public boolean isUsernameTaken(String username) {
        String sql = "SELECT COUNT(*) FROM Expert WHERE username = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, username);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public boolean isEmailTaken(String email) {
        String sql = "SELECT COUNT(*) FROM Expert WHERE email = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
